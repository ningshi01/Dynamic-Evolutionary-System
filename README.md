...
构建一个云原生软件系统动态演化框架，目前具体表现形式为自适应RAG对话系统，
基于Kubernetes，要求支持微服务副本调整、服务拓扑调整、数据流优化等≥3 种系统演化能力。
该系统将各RAG组件微服务化，实现基础设施资源的智能调度与分配，支持服务弹性伸缩，并通过完整的监控体系展示系统动态演化过程。
本项目当前内嵌服务为一个完整的多服务检索增强生成 (RAG) 系统，旨在 Kubernetes 上运行。
它包括文件存储 (MinIO)、文档解析、文本嵌入、重排序、语言模型服务 (LLM) 和统一 API 网关等服务。
