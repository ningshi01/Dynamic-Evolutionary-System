...
_COSINE_INDEX_PARAMS = {
    "index_type": "HNSW",
    "metric_type": "COSINE",
    "params": {"M": 16, "efConstruction": 200}
}
_COSINE_SEARCH_PARAMS = {"metric_type": "COSINE", "params": {"ef": 64}}
...
def get_vector_store(collection_name: str) -> Milvus:
    ...
    vs = Milvus(
        ...
        text_field="text",
        vector_field="vector",
        index_params=_COSINE_INDEX_PARAMS,
        search_params=_COSINE_SEARCH_PARAMS,
    )
...
def create_collection(request: CreateCollectionRequest):
    ...
    col = Collection(name=full_collection_name, schema=schema)

    # 显式创建 COSINE 索引，防止 LangChain add_documents 时用默认 L2 自动建索引
    try:
        col.create_index(
            field_name="vector",
            index_params={
        ...
        logger.warning(f"Failed to create index for collection {full_collection_name}: {idx_e}")
